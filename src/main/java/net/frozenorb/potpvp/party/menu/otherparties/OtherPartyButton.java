...
final class OtherPartyButton extends Button {
    ...
    public String getName(Player player) {
        return ChatColor.DARK_PURPLE + PotPvPSI.getInstance().uuidCache.name(party.getLeader());
    }
    ...
    public List<String> getDescription(Player player) {
        ...
        for (UUID member : party.getMembers()) {
            ...
            description.add(color + PotPvPSI.getInstance().uuidCache.name(member));
        }
    ...
    @Override
    public void clicked(Player player, int slot, ClickType clickType, InventoryView inventoryView) {
