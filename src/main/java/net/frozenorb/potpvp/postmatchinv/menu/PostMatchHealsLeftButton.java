...
final class PostMatchHealsLeftButton extends Button {
    ...
    public List<String> getDescription(Player player) {
        return ImmutableList.of(
                ChatColor.YELLOW + PotPvPSI.getInstance().uuidCache.name(this.player) + " had " + healsRemaining + " " + (healsRemaining == 1 ? healingMethod.getLongSingular() : healingMethod.getLongPlural()) + " left.",
                ChatColor.YELLOW + PotPvPSI.getInstance().uuidCache.name(this.player) + " missed " + missedHeals + " health potion" + (missedHeals == 1 ? "." : "s.")
        );
    }
