...
final class PostMatchSwapTargetButton extends Button {
    ...
    public String getName(Player player) {
        return ChatColor.GREEN + "View " + PotPvPSI.getInstance().uuidCache.name(newTarget.getPlayerUuid()) + "'s inventory";
    }
    ...
    public List<String> getDescription(Player player) {
        return ImmutableList.of(
                "",
                ChatColor.YELLOW + "Swap your view to " + PotPvPSI.getInstance().uuidCache.name(newTarget.getPlayerUuid()) + "'s inventory"
        );
    }
    ...
    @Override
    public void clicked(Player player, int i, ClickType clickType, InventoryView inventoryView) {
