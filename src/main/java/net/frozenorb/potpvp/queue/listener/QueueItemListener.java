...
//                }
                ...
                for (UUID member : party.getMembers()) {
                    if (!RankedMatchQualificationListener.isQualified(member)) {
                        ...
                        player.sendMessage(ChatColor.RED + "Your party can't join ranked queues because " + PotPvPSI.getInstance().uuidCache.name(member) + " has less than " + RankedMatchQualificationListener.MIN_MATCH_WINS + " unranked 1v1 wins. They need " + needed + " more wins!");
                        return;
                    }
                }
            }
