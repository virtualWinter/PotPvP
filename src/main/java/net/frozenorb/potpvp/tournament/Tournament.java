...
public class Tournament {
    ...
    public void check() {
        ...
        if (matches.stream().anyMatch(s -> s != null && s.getState() != MatchState.TERMINATED))
            return; // We don't want to advance to the next round if any matches are ongoing
        matches.clear();
    ...
    private void broadcastJoinMessage(Party joiningParty) {
        if (getCurrentRound() != -1) {
            ...
            message.tooltip(ChatColor.translateAlternateColorCodes('&', "&5Donators &7can join during the tournament countdown. Purchase a rank at &5 " + "store.arcane.cc" + " &7."));
